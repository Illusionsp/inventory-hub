...
router.post("/customers", requireAuth, async (req, res): Promise<void> => {
  ...
  const [c] = await db.insert(customersTable).values({ name, type, email: email ?? null, phone: phone ?? null, address: address ?? null, taxNumber: taxNumber ?? null }).returning();
...
router.patch("/customers/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  for (const f of ["name", "type", "email", "phone", "address", "taxNumber", "isActive"])
