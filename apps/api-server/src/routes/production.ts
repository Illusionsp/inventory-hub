...
router.post("/production-batches", requireAuth, requirePermission("can_create_batch_production"), async (req, res): Promise<void> => {
  ...
  // Wrap creation in a transaction to prevent partial state
  try {
    const result = await db.transaction(async (tx) => {
      const batchNumber = await nextBatchNumber();
      const [batch] = await tx.insert(productionBatchesTable).values({
      ...
      for (const mat of (inputMaterials ?? [])) {
        await tx.insert(productionInputsTable).values({ batchId: batch.id, productId: mat.productId, quantity: mat.quantity.toString(), unit: mat.unit });
        
        // Deduct from source store
        const [inv] = await tx.select().from(inventoryTable).where(and(eq(inventoryTable.productId, mat.productId), eq(inventoryTable.storeId, stageFromStoreId)));
        if (inv) {
          const newQty = Math.max(0, parseFloat(inv.quantity as string) - parseFloat(mat.quantity.toString())).toString();
          await tx.update(inventoryTable).set({ quantity: newQty, updatedAt: new Date() }).where(eq(inventoryTable.id, inv.id));
        }
        await tx.insert(inventoryMovementsTable).values({ productId: mat.productId, storeId: stageFromStoreId, movementType: "production_input", quantity: (-parseFloat(mat.quantity.toString())).toString(), referenceId: batch.id, referenceType: "production_batch", createdBy: req.session.userId });
      }

      const inputs = await tx.select().from(productionInputsTable).where(eq(productionInputsTable.batchId, batch.id));
      return { batch, inputs };
    });
    
    res.status(201).json({ ...result.batch, responsibleUserName: null, inputMaterials: result.inputs, outputProducts: [] });
  } catch (error) {
...
router.post("/production-batches/:id/complete", requireAuth, requirePermission("can_approve_production"), async (req, res): Promise<void> => {
  ...
  const [batch] = await db.select().from(productionBatchesTable).where(eq(productionBatchesTable.id, id));
  if (!batch) { res.status(404).json({ error: "Not found" }); return; }
  if (batch.status === "completed") { res.status(400).json({ error: "Batch is already completed" }); return; }
  ...
  try {
    // Wrap entire completion logic in a transaction to prevent ledger corruption
    const { updated, inputs, outputs } = await db.transaction(async (tx) => {
      const [updatedBatch] = await tx.update(productionBatchesTable).set({
        status: "completed",
      ...
      for (const out of (outputProducts ?? [])) {
        await tx.insert(productionOutputsTable).values({ batchId: id, productId: out.productId, quantity: out.quantity.toString(), unit: out.unit });
        
        const [inv] = await tx.select().from(inventoryTable).where(and(eq(inventoryTable.productId, out.productId), eq(inventoryTable.storeId, batch.stageToStoreId)));
        if (inv) {
          const newQty = (parseFloat(inv.quantity as string) + parseFloat(out.quantity.toString())).toString();
          await tx.update(inventoryTable).set({ quantity: newQty, updatedAt: new Date() }).where(eq(inventoryTable.id, inv.id));
        } else {
          await tx.insert(inventoryTable).values({ productId: out.productId, storeId: batch.stageToStoreId, quantity: out.quantity.toString() });
        }
        await tx.insert(inventoryMovementsTable).values({ productId: out.productId, storeId: batch.stageToStoreId, movementType: "production_output", quantity: out.quantity.toString(), referenceId: id, referenceType: "production_batch", createdBy: req.session.userId });
      }
      ...
      if (hasPackaging) {
        ...
        const [foundPkg] = await tx.select({ id: productsTable.id }).from(productsTable).where(ilike(productsTable.name, pkgProductName));
        if (foundPkg) {
        ...
        if (bulkOutputs.length > 0) {
          ...
          const bulkUnit = bulkOutputs[0].unit as string;
          ...
          for (const out of bulkOutputs) {
            ...
            await tx.insert(inventoryMovementsTable).values({ productId: out.productId, storeId: batch.stageToStoreId, movementType: "packaging_input", quantity: (-deduct).toString(), referenceId: id, referenceType: "production_batch", createdBy: req.session.userId });
          }
        }

        // Add packaged units to inventory
        const [pkgInv] = await tx.select().from(inventoryTable).where(and(eq(inventoryTable.productId, pkgProductId), eq(inventoryTable.storeId, batch.stageToStoreId)));
        if (pkgInv) {
          const newQty = (parseFloat(pkgInv.quantity as string) + pkgCount).toString();
          await tx.update(inventoryTable).set({ quantity: newQty, updatedAt: new Date() }).where(eq(inventoryTable.id, pkgInv.id));
        } else {
          await tx.insert(inventoryTable).values({ productId: pkgProductId, storeId: batch.stageToStoreId, quantity: pkgCount.toString() });
        }
        await tx.insert(inventoryMovementsTable).values({ productId: pkgProductId, storeId: batch.stageToStoreId, movementType: "packaging_output", quantity: pkgCount.toString(), referenceId: id, referenceType: "production_batch", createdBy: req.session.userId });
      }

      const finalInputs = await tx.select().from(productionInputsTable).where(eq(productionInputsTable.batchId, id));
      const finalOutputs = await tx.select().from(productionOutputsTable).where(eq(productionOutputsTable.batchId, id));

      return { updated: updatedBatch, inputs: finalInputs, outputs: finalOutputs };
    });
    ...
    console.error("Production completion transaction failed:", error);
    res.status(500).json({ error: "Failed to complete production batch due to an internal server error" });
  }
});
  ...
  try {
    const { updated, inputs, outputs } = await db.transaction(async (tx) => {
      ...
      for (const mv of outboundMovements) {
        ...
        const [fromInv] = await tx.select().from(inventoryTable).where(
        ...
        if (fromInv) {
          ...
          await tx.update(inventoryTable).set({ quantity: newQty, updatedAt: new Date() }).where(eq(inventoryTable.id, fromInv.id));
        }
        ...
        // Add to target store
        const [toInv] = await tx.select().from(inventoryTable).where(
        ...
        if (toInv) {
          ...
          await tx.update(inventoryTable).set({ quantity: newQty, updatedAt: new Date() }).where(eq(inventoryTable.id, toInv.id));
        } else {
          await tx.insert(inventoryTable).values({ productId: mv.productId!, storeId: target, quantity: qty.toString() });
        }
        await tx.insert(inventoryMovementsTable).values({
      ...
      const [updatedBatch] = await tx.update(productionBatchesTable)
    ...
    res.status(500).json({ error: "Failed to dispatch due to an internal server error" });
  }
});
