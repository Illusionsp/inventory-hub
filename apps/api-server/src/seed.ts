...
async function seed() {
  ...
  if (existingCats.length === 0) {
    await db.insert(categoriesTable).values([
      ...
      { name: "Packaging Materials", description: "Materials used for packaging" },
  ...
  if (existingInventory.length === 0) {
    await db.insert(inventoryTable).values([
      { productId: bysku("RM-001").id, storeId: rawStore.id, quantity: "1200" },
      { productId: bysku("RM-002").id, storeId: rawStore.id, quantity: "450" },
      { productId: bysku("RM-003").id, storeId: rawStore.id, quantity: "85" },
      { productId: bysku("SF-001").id, storeId: semiStore.id, quantity: "300" },
      { productId: bysku("FP-001").id, storeId: finishedStore.id, quantity: "850" },
      { productId: bysku("FP-002").id, storeId: finishedStore.id, quantity: "320" },
      { productId: bysku("PK-001").id, storeId: rawStore.id, quantity: "200" },
      { productId: bysku("PK-002").id, storeId: rawStore.id, quantity: "800" },
      { productId: bysku("PK-003").id, storeId: rawStore.id, quantity: "2000" },
    ]);
